...
export function CuentasPorPagarClient({ cuentas, totales, puedeGestionar }: { cuentas:Cuenta[]; totales:Totales; puedeGestionar:boolean }) {
  ...
  const [fecha, setFecha] = useState(hoyRD()); const [error, setError] = useState<string|null>(null); const [guardando, setGuardando] = useState(false); const [historial,setHistorial]=useState<{cuenta:Cuenta;pagos:Pago[]}|null>(null);
  ...
  return <Box component="section" sx={{ p:{xs:2,sm:3}, maxWidth:1100, mx:'auto', display:'flex', flexDirection:'column', gap:2 }}>
    ...
    <Box sx={{border:'1px solid #e5e7eb',borderRadius:2,bgcolor:'#fff',overflowX:'auto'}}><Box component="table" sx={{width:'100%',minWidth:760,borderCollapse:'collapse'}}>
      ...
      <Box component="tbody">{cuentas.map(c=><Box component="tr" key={c.id} sx={{borderTop:'1px solid #f3f4f6'}}><Box component="td" sx={{p:1.25,fontSize:14}}>{c.proveedorNombre??'Proveedor sin nombre'}</Box><Box component="td" sx={{p:1.25,fontFamily:'monospace',fontSize:12}}>{c.referenciaEncf??`#${c.id}`}</Box><Box component="td" sx={{p:1.25,fontSize:12,color:c.vencida?'#dc2626':'#374151'}}>{c.fechaVencimiento??'—'}{c.vencida?` · ${c.diasVencido}d`:''}</Box><Box component="td" sx={{p:1.25,textAlign:'right'}}>{fmtDOP(c.montoTotal)}</Box><Box component="td" sx={{p:1.25,textAlign:'right'}}>{fmtDOP(c.pagado)}</Box><Box component="td" sx={{p:1.25,textAlign:'right',fontWeight:700}}>{fmtDOP(c.saldo)}</Box><Box component="td" sx={{p:1.25,textAlign:'right'}}><Button size="small" title="Historial de pagos" onClick={()=>verHistorial(c)}><History size={14}/></Button>{puedeGestionar&&<Button size="small" startIcon={<Wallet size={14}/>} onClick={()=>{setPago(c);setMonto((c.saldo/100).toFixed(2));setError(null)}}>Pagar</Button>}</Box></Box>)}{!cuentas.length&&<Box component="tr"><Box component="td" colSpan={7} sx={{p:4,textAlign:'center',color:'#6b7280'}}>No hay compras a crédito pendientes.</Box></Box>}</Box>
    </Box></Box>
