          ...
          <Table size="small" sx={{ width: '100%', '& tbody td': { borderBottom: '1px solid #f3f4f6' }, '& tbody tr:last-child td': { borderBottom: 0 } }}>
            <TableHead>
              <TableRow sx={{ bgcolor: '#f9fafb' }}>
                {([['#', 'left'], ['Usuario', 'left'], ['Facturas', 'right'], ['Facturado', 'right'], ['%', 'right']] as const).map(([h, align]) => (
            ...
            <TableBody>
              {filas.length === 0 ? (
                <TableRow><TableCell colSpan={5} sx={{ px: 2, py: 4, textAlign: 'center', color: '#9ca3af' }}>Sin ventas en este rango.</TableCell></TableRow>
              ) : filas.map((f, i) => (
                <TableRow key={f.usuarioId ?? `nn-${i}`} sx={{ '&:hover': { bgcolor: '#f9fafb' } }}>
                  ...
                  <TableCell align="right" sx={{ px: 2, py: 1.5, color: '#374151', fontVariantNumeric: 'tabular-nums' }}>{f.numFacturas}</TableCell>
                  <TableCell align="right" sx={{ px: 2, py: 1.5, color: '#111827', fontWeight: 500, fontVariantNumeric: 'tabular-nums' }}>{fmtDOP(f.ingresosCents)}</TableCell>
                  <TableCell align="right" sx={{ px: 2, py: 1.5, color: '#6b7280', fontVariantNumeric: 'tabular-nums' }}>{total > 0 ? Math.round(f.ingresosCents / total * 100) : 0}%</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
