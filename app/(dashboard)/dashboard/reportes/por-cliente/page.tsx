    ...
    >
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-200">
          <h2 className="text-sm font-semibold text-gray-900">Detalle por cliente ({filas.length})</h2>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
              <tr>
                ...
                <th className="px-4 py-2.5 text-left">RNC</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filas.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400">Sin ventas en este rango.</td></tr>
              ) : filas.map((f, i) => (
                <tr key={`${f.cliente}-${f.rnc ?? i}`} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-400">{i + 1}</td>
                  <td className="px-4 py-3 text-gray-900 font-medium">{f.cliente}</td>
                  <td className="px-4 py-3 text-gray-400 text-xs">{f.rnc ?? '—'}</td>
