...
export default async function RegistrarCompraPage({ searchParams }: { searchParams: Promise<{ ecf?: string; captura?: string }> }) {
  ...
  const capturaId = Number(captura);
  return <RegistrarCompraClient contexto={await contextoRegistro(teamId, 'compra', ecf ?? null, Number.isInteger(capturaId) && capturaId > 0 ? capturaId : null)} />;
}
