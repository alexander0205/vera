...
const NOMINA: { campo: keyof ConfigContable; label: string; ayuda: string }[] = [
  ...
  { campo: 'cuentaNominaAportesGastoId', label: 'Gasto de aportes patronales',
    ayuda: 'Lo que aporta la empresa a la TSS (AFP, SFS, SRL) y al INFOTEP, como gasto (Debe).' },
  { campo: 'cuentaNominaRetencionesId', label: 'Retenciones TSS por pagar',
    ayuda: 'AFP, SFS y dependientes que le retienes al empleado y le debes a la TSS (Haber).' },
  { campo: 'cuentaNominaIsrPagarId', label: 'ISR de asalariados por pagar',
    ayuda: 'El ISR retenido que se le paga a la DGII (Haber). Vacío: usa la de retenciones TSS.' },
  { campo: 'cuentaNominaAportesPagarId', label: 'Aportes patronales TSS por pagar',
    ayuda: 'AFP, SFS y SRL de la empresa que quedan por pagar a la TSS (Haber).' },
  { campo: 'cuentaNominaInfotepPagarId', label: 'INFOTEP por pagar',
    ayuda: 'El 1 % del INFOTEP por pagar (Haber). Vacío: usa la de aportes TSS.' },
  { campo: 'cuentaNominaPorPagarId', label: 'Sueldos netos por pagar',
    ayuda: 'El neto que se le debe al empleado hasta que se le paga (Haber).' },
];

/** Una cuenta de gasto y una de pasivo por cada provisión. Vacías → las dos generales. */
const PROVISIONES: { campo: keyof ConfigContable; label: string; ayuda: string }[] = [
  { campo: 'cuentaProvRegaliaGastoId', label: 'Regalía pascual · gasto', ayuda: 'Debe mensual de la regalía.' },
  { campo: 'cuentaProvRegaliaPagarId', label: 'Regalía pascual · pasivo', ayuda: 'Lo que se acumula hasta pagarla en diciembre.' },
  { campo: 'cuentaProvVacacionesGastoId', label: 'Vacaciones · gasto', ayuda: 'Debe mensual de las vacaciones.' },
  { campo: 'cuentaProvVacacionesPagarId', label: 'Vacaciones · pasivo', ayuda: 'Las vacaciones ganadas que faltan por pagar.' },
  { campo: 'cuentaProvCesantiaGastoId', label: 'Cesantía · gasto', ayuda: 'Debe mensual de la cesantía.' },
  { campo: 'cuentaProvCesantiaPagarId', label: 'Cesantía · pasivo', ayuda: 'La reserva para prestaciones.' },
];
...
export function ConfigClient({
  ...
  async function aplicarRecomendada() {
    setGuardando(true);
    setError(null);
    setAviso(null);
    const res = await fetch('/api/contabilidad/config', {
      ...
      body: JSON.stringify({ seccion: 'recomendada' }),
    });
    setGuardando(false);
    const body = await res.json().catch(() => ({}));
    if (!res.ok) {
      setError(body.error ?? 'No se pudo aplicar la configuración recomendada.');
      return;
    }
    const partes = [
      body.cuentasCreadas > 0 ? `${body.cuentasCreadas} cuenta(s) nuevas en el catálogo` : null,
      `${body.camposConfigurados} cuenta(s) asignadas`,
      `${body.metodosConfigurados} forma(s) de cobro`,
    ].filter(Boolean);
    setAviso(`Listo: ${partes.join(', ')}. Revisa y enciende la contabilidad cuando esté a tu gusto.`);
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
      ...
      {puedeConfigurar && (
        <Box component="section" sx={{ ...CARD, p: 2.5, display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 2 }}>
          <Box sx={{ minWidth: 0, flex: '1 1 320px' }}>
            <Typography component="h2" sx={{ fontSize: '0.875rem', fontWeight: 600, color: '#111827' }}>
              Configuración recomendada
            </Typography>
            <Typography sx={{ fontSize: '0.75rem', color: '#6b7280' }}>
              Llena todo lo que está sin configurar con las cuentas estándar del catálogo —ventas, compras, nómina,
              provisiones y formas de cobro— y crea las que falten. No cambia lo que ya elegiste ni enciende nada.
            </Typography>
          </Box>
          <Button variant="contained" size="small" disabled={guardando} onClick={aplicarRecomendada}>
            Completar con la recomendada
          </Button>
        </Box>
      )}
      ...
      <Box component="section" sx={{ ...CARD, p: 2.5, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
        <Box>
          ...
          <Typography sx={{ fontSize: '0.75rem', color: '#6b7280' }}>
            Gravado: el ITBIS de las facturas con crédito fiscal (B01, E31) se adelanta en 1104. Exento: la empresa no cobra ITBIS
            y el de sus compras se suma al costo. Consumo, gastos menores y pagos al exterior van siempre al costo.
          </Typography>
        </Box>
        <TextField
        ...
        >
          <MenuItem value="exento">Exento — el ITBIS de las compras va al costo</MenuItem>
          <MenuItem value="gravado">Gravado — el ITBIS se adelanta en 1104</MenuItem>
        </TextField>
      </Box>
          ...
          {configInicial.provisionarNomina && (
            <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)' } }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>
                ...
                <Typography sx={{ fontSize: '0.75rem', color: '#6b7280' }}>Para las provisiones sin cuenta propia (Debe). Vacío: usa la de gastos.</Typography>
              </Box>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>
                <Typography component="label" sx={{ fontSize: '0.875rem', fontWeight: 500, color: '#374151' }}>
                ...
                <Typography sx={{ fontSize: '0.75rem', color: '#6b7280' }}>Para las provisiones sin cuenta propia (Haber). Vacío: usa la de por pagar.</Typography>
              </Box>
            </Box>
          )}
        </Box>
