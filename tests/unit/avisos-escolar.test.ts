...
describe('avisosDeHoy', () => {
  ...
  describe('ruteo de canales', () => {
    ...
    it('los dos del final no salen por WhatsApp: la ventana de 24h no los sostiene', () => {
      expect(avisosDeHoy(fila(), '2026-10-03')[0].canales).toEqual(['correo', 'sms']);
      expect(avisosDeHoy(fila(), '2026-10-06')[0].canales).toEqual(['correo', 'sms']);
    });

    it('un canal apagado se cae del ruteo', () => {
      const f = fila({ avisoSms: false });
      expect(avisosDeHoy(f, '2026-10-06')[0].canales).toEqual(['correo']);
    });

    it('si ningún canal del aviso está encendido, el aviso no existe', () => {
      // Con solo WhatsApp, el aviso del vencimiento —que va por correo y SMS—
      // no tiene por dónde salir y no debe contarse como pendiente.
  ...
  describe('el interruptor maestro del colegio', () => {
    ...
    it('el canal apagado no sale aunque el concepto lo tenga encendido', () => {
      ...
      expect(avisosDeHoy(fila(), '2026-10-06', canales)[0].canales).toEqual(['sms']);
    });
